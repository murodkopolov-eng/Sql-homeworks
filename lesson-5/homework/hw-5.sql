...
SELECT ProductName as Name FROM Products ORDER BY Name
...
--5)Write a query select distinct customer names and their corresponding country using SELECT DISTINCT.
...
SELECT StockQuantity,
                    IIF(StockQuantity>100,'YES','NO') AS Result FROM Products_Discounted

--8)Use UNION to combine results from two queries that select ProuctName from Products and ProductName Products_Discounted tables.

SELECT ProductName FROM Products
UNION
SELECT ProductName FROM Products_Discounted

--9)Write a query that returns the difference between the Products and Products_Discounted table using EXCEPT.
...
SELECT Price,
			IIF(Price>1000,'Expensive','Affordable') AS Result FROM Products

--11)Write a query to find employees in the Employees table who have either Age<25 OR Salary>6000.

SELECT Age,Salary FROM Employees WHERE Age < 25 OR Salary > 60000
...
--13)Write a query that uses CASE to assign 'Top Fier' if SaleAmount>500,'Mid Tier' if SaleAmount BETWEEN 200 AND 500,and 'Low Tier' otherwise.(From sales table)

SELECT * FROM Sales

SELECT SaleAmount,
				 CASE
	   WHEN SaleAmount>500 THEN 'Top Fier'
...
--15)Write a query that uses a CASE statement to determine the discount percentage based on the quantity purchased. Use orders table.
...
SELECT CustomerID,Quantity,
				 ...
				 ELSE 0.07 END AS DiscountPercentage FROM Orders
