...
CROSS APPLY (
			 ...
			 FROM Customers c
			 WHERE c.CustomerID=o.CustomerID AND YEAR(o.OrderDate)>2022) AS c
...
CROSS APPLY (
			 SELECT Name
			 FROM Employees e 
			 WHERE d.DepartmentID=e.DepartmentID AND d.DepartmentName IN ('Sales','Marketing')) AS e

--3)Return: DepartmentName, MaxSalary
--Task: Show the highest salary for each department.
--Tables Used: Departments, Employees

SELECT d.DepartmentName,e.MaxSalary
FROM Departments d
CROSS APPLY (
			 SELECT TOP 1 MAX(e.Salary) AS MaxSalary
			 FROM Employees e
			 WHERE d.DepartmentID=e.DepartmentID) AS e
...
CROSS APPLY (
			 ...
			 AND c.Country='USA'AND YEAR(o.OrderDate)=2023) AS c
...
CROSS APPLY (
			 ...
			 FROM Orders o
			 WHERE o.CustomerID=c.CustomerID) AS o
...
CROSS APPLY (
			 ...
			 AND s.SupplierName IN ('Gadget Supplies','Clothing Mart')) AS p
...
SELECT * FROM Orders
SELECT c.FirstName,c.LastName,o.MostRecentOrderDate
FROM Customers c
OUTER APPLY (
			 SELECT MAX(o.OrderDate) AS MostRecentOrderDate
			 FROM Orders o
			 WHERE o.CustomerID=c.CustomerID) AS o 
...
FROM Customers c
CROSS APPLY (
			 SELECT TotalAmount AS OrderTotal
			 FROM Orders o
			 WHERE o.CustomerID=c.CustomerID
			 AND o.TotalAmount>500) AS o
...
CROSS APPLY (
			 ...
			 WHERE c.CustomerID=o.CustomerID
			 AND c.Country='USA'
			 AND c.Address LIKE ('[0-9][0-9][0-9][0-9]%')) AS c
...
CROSS APPLY (
			 SELECT p.ProductID
			 FROM  Products p
			 WHERE p.Category=c.CategoryID) AS p
