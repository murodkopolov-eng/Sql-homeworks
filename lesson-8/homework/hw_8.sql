...
SELECT StockQuantity,
		...
		ELSE 'Sufficient' END AS Result FROM Products

--8)Using Customers table, find the total number of customers in each country.

SELECT Country, COUNT(CustomerID) AS TotalCustomer FROM Customers
GROUP BY Country
...
SELECT Year(OrderDate) AS OrderYear, AVG(Quantity) AS AvgQuantity FROM Orders
...
SELECT district_id, district_name,year, [2012], [2013] 
INTO Population_Each_Year 
         FROM (
			   SELECT district_id,district_name,population,year FROM city_population) AS SouceTable
		PIVOT (
		       MAX(Population) FOR year IN ([2012],[2013])) AS PivotTable
...
SELECT district_id,year,[Bektemir],[Chilonzor],[Yakkasaroy] INTO Population_Each_City FROM ( SELECT district_id,year,district_name,population FROM city_population) AS SourceTable
PIVOT ( Max(population) FOR	district_name IN ([Bektemir],[Chilonzor],[Yakkasaroy])) AS PivotTable
...
SELECT district_id,population,year,district_name
