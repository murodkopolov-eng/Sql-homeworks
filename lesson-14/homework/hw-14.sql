...
SELECT ID,LEN(Vals) - LEN(REPLACE(Vals,'.','')) AS Result FROM testDots
WHERE LEN(Vals) - LEN(REPLACE(Vals,'.',''))>2

--5)Write a SQL query to count the spaces present in the string.(CountSpaces)

SELECT LEN(texts) - LEN(REPLACE(texts,' ','')) AS Result FROM CountSpaces
...
WITH Compare AS (
				 ...
				 RecordDate,
				 Temperature,
				 LAG(Temperature)OVER(ORDER BY RecordDate) AS Previous_Temp FROM weather)
				 SELECT * FROM Compare
...
WITH CompareCTE AS (
				   ...
				   device_id,
				   event_date,
				   ROW_NUMBER()OVER(PARTITION BY player_id ORDER BY event_date ASC) AS RowNumber FROM Activity)
				   SELECT * FROM CompareCTE
				   WHERE RowNumber=1
...
WITH OneFruitCTE AS (
					 ...
					 SELECT * FROM OneFruitCTE WHERE RowNumber=3
...
SELECT 
	  ...
	  DATEDIFF(YEAR,HIRE_DATE,GETDATE()) AS YearsOfService,
	          CASE 
				  ...
				  ELSE 'Veteran' END AS Result FROM Employees
...
SELECT 
	  ID,
	  VALS,
	  CAST(SUBSTRING(VALS,1,PATINDEX('%[^0-9]%',VALS + 'a') - 1) AS INT) AS Number FROM GetIntegers
...
DECLARE @Vals NVARCHAR(100)='a.b.c,x.y.z'

WITH SplitVals AS (
				   SELECT
				   Id,Vals,
                   ROW_NUMBER()OVER(ORDER BY (SELECT 1)) AS RowNumber FROM STRING_SPLIT(@Vals,',')),
   SwappedWals AS  (
                   SELECT 
				   RowNumber,
				             CASE
							     WHEN 
									 ...
									 ELSE @Vals END AS Result FROM SplitVals
...
WITH  StringCTE AS (
							   SELECT 1 AS ID,
							   SUBSTRING('sdgfhsdgfhs@121313131',1,1) AS Result
							   UNION ALL
							   SELECT ID + 1,
							   SUBSTRING('sdgfhsdgfhs@121313131',ID + 1,1) 
							   FROM StringCTE
							       WHERE ID < LEN('sdgfhsdgfhs@121313131'))
							   SELECT ID,Result FROM StringCTE
...
WITH CompareCTE AS (
					...
					device_id,
					event_date,
				    ROW_NUMBER()OVER(PARTITION BY player_id ORDER BY event_date) AS RowNumber FROM Activity)
					SELECT * FROM CompareCTE
					WHERE RowNumber=1
...
WITH  StringCTE1 AS (
							   SELECT 1 AS ID,
							   SUBSTRING('rtcfvty34redt',1,1) AS Result
							   UNION ALL
							   SELECT ID + 1,
							   SUBSTRING('rtcfvty34redt',ID + 1,1) 
							   FROM StringCTE1
							   WHERE ID < LEN('rtcfvty34redt'))
                               SELECT
							         STRING_AGG(CASE WHEN Result LIKE '[0-9]' THEN Result ELSE '' END,'') AS Integers,
									 STRING_AGG(CASE WHEN Result LIKE '[a-z]' THEN Result ELSE '' END,'') AS Characters
									 FROM StringCTE1
