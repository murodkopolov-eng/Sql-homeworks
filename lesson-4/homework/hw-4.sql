...
--3)Write a query that filters the Prodcuts table to show products with Price>100
...
--4)Write a query to select all Customers whose FirstName start with 'A' using LIKE operator.
...
--5)Order the results of a Products table in ascending order

SELECT * FROM Products
ORDER BY Price ASC

--6)Write a query that uses the WHERE clause to filter for Employees with Salary>=6000 and DepartmentName 'HR'
...
SELECT FirstName,ISNULL(Email,'noemail@example.com') AS Email FROM Employees

--8)Write a query that show all products with Price BETWEEN 50 AND 100.

SELECT * FROM Products WHERE Price BETWEEN 50AND 100


--9)Use SELECT DISTINCT on two columns (Category and ProductName) in the Producrs Table.
...
--11)Write a query to select top 10 products from the Products table,ordered by Price DESC.

SELECT TOP 10 * FROM Products ORDER BY Price DESC

 --12)Use COALESCE to return the first non-NULL value from FirstName  or LastName in the Employees table.

 SELECT EmployeeID,COALESCE(FirstName,LastName) AS FullName FROM Employees

 --13)Write a query that selects distinct Category and Price from Products table.
 ...
 --16)Write a query to display all prodcuts with Price<=1000 and StockQuantity>50,sorted by Stock in acsending order.

 SELECT * FROM Products WHERE Price<=1000 AND StockQuantity>50 ORDER BY StockQuantity ASC

 --17)Write a query that filters the Producs table for ProductName values containing the letter 'e' using like.

 SELECT * FROM Products WHERE ProductName LIKE '%e%'
 SELECT * FROM Products

 --18)Use IN operator to filter for employees who in either	'HR','IT', OR 'Finance'.
...
SELECT City,PostalCode  FROM Customers ORDER BY City ASC,PostalCODE DESC
...
--22)Write a quaery to select the distinct Category,ProductName,and Price for products that are priced above $50,using distinct on three columns.
...
--26)Write a query that uses ALL operator to find employees whose salary is greate than all employees in the 'Sales' department.
...
--27)Write a query that filters Orders table for orders placed in the last 180 days using BETWEEN and LATEST_DATE in the table.(Search how to get the current date and latest date)
