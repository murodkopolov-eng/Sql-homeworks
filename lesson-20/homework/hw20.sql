...
SELECT * FROM (
			   SELECT TOP 1 Product,SUM( Price * Quantity ) AS TotalRevenue
			   FROM #Sales
			   GROUP BY Product
			   ORDER BY TotalRevenue DESC ) AS t
...
SELECT * FROM (
			   SELECT 
			         Product,
					 SUM( Price * Quantity ) AS Second_Highest_SaleAmount,
					 RANK() OVER ( ORDER BY SUM( Price * Quantity ) DESC ) AS Rank7 
					 FROM #Sales
...
SELECT * FROM (
		       SELECT MONTH( SaleDate ) AS Month,SUM( Quantity ) AS NumberOFProducts
			   FROM #Sales
...
SELECT * FROM (
			   SELECT Name,Fruit,COUNT( Fruit ) AS Number_Of_Fruits FROM Fruits
			   GROUP BY Name,Fruit ) AS t
PIVOT (
       MAX( Number_Of_Fruits ) FOR Fruit IN ([Apple],[Orange],[Banana])) AS PivotTable

--7. Return older people in the family with younger ones

SELECT b.ParentId, a.ChildID
FROM Family AS a
INNER JOIN Family AS b ON b.ParentId < a.ChildID
...
SELECT 
      *,
	  DENSE_RANK() OVER ( ORDER BY Vals ) AS rwn
FROM #RankingPuzzle

--12. Find employees whose sales were higher than the average sales in their department

SELECT * FROM #EmployeeSales AS e1
WHERE SalesAmount > ( SELECT AVG( SalesAmount ) FROM #EmployeeSales AS e2 WHERE e1.Department = e2.Department )
...
SELECT * FROM #EmployeeSales AS e1
WHERE EXISTS (
			  SELECT 1 FROM #EmployeeSales AS e2
...
WHERE Price > ( SELECT MIN(Price) FROM Products AS p2 WHERE p2.Category = 'Electronics' )
...
WITH CTE_RESULT AS (
                    SELECT * FROM Orders
					WHERE Quantity > ( SELECT  AVG( Quantity ) AS AvgQ FROM Orders ))
					SELECT p.Name,c.Quantity FROM CTE_RESULT AS C
...
SELECT 
      TOP 1 p.Name,
	  SUM( o.Quantity ) AS MaxTotalQuantity
FROM Products AS p
