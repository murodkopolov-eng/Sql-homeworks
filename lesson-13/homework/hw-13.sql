...
SELECT CONCAT(EMPLOYEE_ID,'-',FIRST_NAME,' ',LAST_NAME) AS FinalResult FROM Employees
...
SELECT SUBSTRING(PHONE_NUMBER,5,3) + REPLACE(PHONE_NUMBER,'124','999') AS FinalResult FROM Employees
...
SELECT FIRST_NAME,LEN(FIRST_NAME) AS LenName FROM Employees 
...
SELECT YEAR1, 
GREATEST(Max1,Max2,Max3) as MaxResult from TestMax
...
SELECT VALS,id FROM SingleOrder
...
SELECT id,COALESCE(ssn,passportid,itin,'NO VALUE') AS Result FROM person
...
SELECT o1.CustomerID,o1.DeliveryState
FROM Orders o1
...
SELECT EMPLOYEE_ID,
	   TEMP.FULLNAME
	   FROM ( 
	         SELECT CONCAT(FIRST_NAME,' ',LAST_NAME) AS FULLNAME, * FROM Employees) AS TEMP
			 WHERE TEMP.FULLNAME LIKE '%a%%a%%a%'
...
SELECT Birthday,STRING_AGG(StudentName,',') AS Result FROM Student
GROUP BY Birthday
...
SELECT
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) LIKE '[A-Z]' THEN SUBSTRING(@str, n, 1) END, '') AS UpperCase,
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) LIKE '[a-z]' THEN SUBSTRING(@str, n, 1) END, '') AS LowerCase,
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) LIKE '[0-9]' THEN SUBSTRING(@str, n, 1) END, '') AS Numbers,
    STRING_AGG(CASE WHEN SUBSTRING(@str, n, 1) NOT LIKE '[A-Za-z0-9]' THEN SUBSTRING(@str, n, 1) END, '') AS Others
FROM Numbers
