...
SELECT DepartmentName, COUNT(DepartmentName) AS TotalEmployees FROM Employees
...
SELECT COUNT(DepartmentName) AS EmployeesHR FROM Employees
...
SELECT DepartmentName, AVG(Salary) AS AvgSalary, COUNT(*) AS TotalEmployees FROM Employees
GROUP BY DepartmentName
...
SELECT Year(SaleDate), SUM(SaleAmount) AS TotalSales FROM Sales
GROUP BY Year(SaleDate)
select * from Sales

--18)Write a query to show the list of customers who placed at least 3 orders.

SELECT CustomerID, COUNT(OrderID) AS Orders FROM Orders
GROUP BY CustomerID
...
SELECT Month(OrderDate), SUM (TotalAmount) AS TotalSales, COUNT(DISTINCT ProductID) AS UniqueProducts FROM Orders
...
SELECT YEAR(OrderDate), MAX(Quantity) AS MaxQty, MIN(Quantity) AS MinQty FROM Orders
